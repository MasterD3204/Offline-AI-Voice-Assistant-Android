            ...
            val config = OfflineRecognizerConfig(
                modelConfig = OfflineModelConfig(
                    transducer = OfflineTransducerModelConfig(
                        encoder = "model/encoder-epoch-20-avg-10.int8.onnx",
                        decoder = "model/decoder-epoch-20-avg-10.int8.onnx",
                        joiner = "model/joiner-epoch-20-avg-10.int8.onnx"
                    ),
                    tokens = "model/tokens.txt",
                    numThreads = 2,
                    debug = true,
                    modelType = "transducer"  // Chỉ định rõ loại model
                ),
                decodingMethod = "greedy_search"  // hoặc "modified_beam_search"
            )
